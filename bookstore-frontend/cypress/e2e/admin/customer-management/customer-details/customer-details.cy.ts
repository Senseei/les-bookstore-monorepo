...
describe('Admin - Customer Details Page', () => {
  ...
  before(() => {
    ...
    cy.createRealUser().then((user) => {
      testUser = user
      cy.log('User created successfully for customer details test:', testUser.id)
    })
  })
  ...
  it('should display personal information section with all fields', () => {
    ...
    cy.contains('Nome Completo').parent().should('contain.text', testUser.name || 'Updated User')
  ...
  it('should validate data formatting and consistency', () => {
    ...
    cy.contains('CPF').parent().invoke('text').should('match', /\d{3}\.\d{3}\.\d{3}-\d{2}/)
    ...
    cy.contains('Data de Nascimento').parent().invoke('text').should('match', /\d{2}\/\d{2}\/\d{4}/)
    cy.contains('Data de Cadastro').parent().invoke('text').should('match', /\d{2}\/\d{2}\/\d{4}/)
